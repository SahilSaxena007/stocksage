...
def train_one(ticker):
    ...
    os.makedirs(f"artifacts/models/{ticker}", exist_ok=True)
    pickle.dump(pipe_cls, open(f"artifacts/models/{ticker}/cls.pkl","wb"))
    pickle.dump(pipe_reg, open(f"artifacts/models/{ticker}/reg.pkl","wb"))
    json.dump(list(X.columns), open(f"artifacts/models/{ticker}/features.json","w"))
